...
export default function OtpVerify() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/auth/verify-otp', { phone, otp: code }, { useAuth: false });
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <p className="text-center text-[12px] text-gray-400">
          Didn't get it? <button type="button" className="text-primary font-medium hover:underline" onClick={() => toast.success('Code resent')}>Resend</button>
        </p>
      </form>
    </div>
  );
